...
```{r}
C212_2 <- drop_na(pivot_longer(C212_2, cols = colnames(C212_2)[2:9]))
```


## Plots

Make multi-panel profile plots for each separate cruise

```{r}
# C212_2
C212_2_panel1 <- ggplot() +
  geom_line(data = subset(C212_2, name %in% c("BAT")), aes(x = depth, y = value, color = "black"), size=1.5, na.rm = T) +
  geom_area(data = subset(C212_2, name %in% c("oxygen")), aes(x = depth, y = value/300, fill = "cyan")) +
...
#C212_2_panel1 <- set_panel_size(C212_2_panel1, width  = unit(3, "cm"), height = unit(6, "cm"))  # set explicit panel size so they will be consistent for all figures. Function from egg package
...
C212_2_panel2 <- ggplot() +
  ...
  geom_area(data = subset(C212_2, name %in% c("oxygen")), aes(x = depth, y = value/8, fill = "cyan")) +
  scale_fill_manual(name = '', values = c('cyan'='cyan'), labels = c('oxygen')) +
  ...
  scale_y_continuous(name = expression(Microbial~Abund~"["~x10^{8}~L^{-1}~"]"),
                     sec.axis = sec_axis(trans=~.*8, name="oxygen [µM]"), expand = c(0, 0)) +
  coord_flip() +
  scale_x_reverse(limits = c(400, 100), expand = c(0, 0)) +
  theme_bw() +
  theme(legend.position = "none") +
...
C212_2_panel3 <- ggplot() +
  geom_line(data = subset(C212_2, name %in% c("NH4")), aes(x = depth, y = value, color="red"), size=1, lty = 1) +
  geom_point(data = subset(C212_2, name %in% c("NH4")), aes(x = depth, y = value, color="red"), shape = 15, size = 3) +
  geom_line(data = subset(C212_2, name %in% c("NO_x")), aes(x = depth, y = value, color="darkgreen"), size=1, lty = 1) +
  geom_point(data = subset(C212_2, name %in% c("NO_x")), aes(x = depth, y = value, color="darkgreen"), shape = 17, size = 3) +
  geom_area(data = subset(C212_2, name %in% c("oxygen")), aes(x = depth, y = value/8, fill = "cyan")) +
  scale_fill_manual(name = '', values = c('cyan'='cyan'), labels = c('oxygen')) +
  scale_colour_manual(name = '', values =c('red'='red','darkgreen'='darkgreen'), labels = c('NH4','NOx')) +
...
# cowplot- put together panels, turning legends off
C212_2_plot <- plot_grid(C212_2_panel1, C212_2_panel2, C212_2_panel3, labels = "AUTO", nrow = 1, hjust = -1)
C212_2_plot

# get legend that is horizontally laid out BLARGHHGHGHGHGGHGH keep working on this: https://stackoverflow.com/questions/52060601/ggplot-multiple-legends-arrangement
legend_1 <- get_legend(
  C212_2_panel1 + 
...
legend123 <- plot_grid(leg12, leg30,
                    ncol = 3)
plot_grid(C212_2_plot, legend_1, legend_2, nrow = 3, ncol = 1, rel_heights = c(1, 1, .1))
