...
C212_2_panel1 <- ggplot(C212_2) +
  ...
  geom_area(aes(y = oxygen/300, x = depth), fill = "cyan") +
...
C212_2_panel2 <- ggplot(C212_2) +
  geom_line(data = C212_2[!is.na(C212_2$Micro_Abund_DAPI),], aes(y = Micro_Abund_DAPI, x = depth), color="black", size=1, na.rm = T, lty = 2) + # subset and only select rows with data, otherwise the NAs mess up the line
  geom_point(aes(y = Micro_Abund_DAPI, x = depth), shape = 21, colour = "black", fill = "white", size = 3) +
  geom_area(aes(y = oxygen/8, x = depth), fill = "cyan") +
...
C212_2_panel3 <- ggplot(C212_2) +
  geom_line(data = C212_2[!is.na(C212_2$NH4),], aes(y = NH4, x = depth), color="red", size=1, na.rm = T, lty = 1) +
  geom_point(aes(y = NH4, x = depth), shape = 15, colour = "red", size = 3) +
  geom_line(data = C212_2[!is.na(C212_2$NO_x),], aes(y = NO_x, x = depth), color="darkgreen", size=1, na.rm = T) +
  geom_point(aes(y = NO_x, x = depth), shape = 17, colour = "darkgreen", size = 3) +
  geom_area(aes(y = oxygen/8, x = depth), fill = "cyan") +
...
C212_2_plot <- plot_grid(C212_2_panel1, C212_2_panel2, C212_2_panel3, labels = "AUTO", nrow = 1)
C212_2_plot

# get legend that is horizontally laid out BLARGHHGHGHGHGGHGH CANNOT GET THIS TO WORK FOR SH*** STOPPED 1/9/20
legend_b <- get_legend(C212_2_panel3)

# add the legend underneath the row we made earlier
plot_grid(C212_2_plot, legend_b, nrow = 2)
